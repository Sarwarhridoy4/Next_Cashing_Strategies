...
flowchart LR
  ...
  B -->|Static Generation (SSG)| C[Pre‑render at build time]
  B -->|Server Side Rendering (SSR)| D[Render per request]
  B -->|Incremental Static Regeneration (ISR)| E[Pre‑render + revalidate]
  B -->|Client‑side Rendering (CSR)| F[Render in browser]
```
